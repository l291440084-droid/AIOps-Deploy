...
from deploy_core import (
    ALTERNATIVE_TOOLS, check_tool_installed,
    install_alternative_tool, uninstall_claude,
)
...
class DeployApp:
    ...
    def _show_page(self, page_id):
        ...
        # On system page, show uninstall if Claude is already installed
        if page_id == "system" and hasattr(self, 'uninstall_frame'):
            claude_check = core.check_claude()
            if claude_check.get("installed"):
                self.uninstall_label.config(
                    text=f"Claude Code {claude_check.get('version', '')} is installed. You can uninstall it:"
                )
                self.uninstall_frame.pack(fill=tk.X, padx=20, pady=(6, 0), before=self.console)
            else:
    ...
    def _build_page_system(self, page):
        ...
        # Uninstall Claude Code section (shown when already installed)
        self.uninstall_frame = tk.Frame(page, bg="#FFEBEE", highlightbackground="#EF9A9A",
                                         highlightthickness=1)
        self.uninstall_label = tk.Label(self.uninstall_frame, text="",
                                         bg="#FFEBEE", fg="#C62828",
                                         font=("Microsoft YaHei", 10),
                                         wraplength=800, justify=tk.LEFT)
        self.uninstall_label.pack(side=tk.LEFT, padx=12, pady=10)
        self.uninstall_btn = ttk.Button(self.uninstall_frame,
                                         text=_("btn_uninstall_claude"),
                                         command=self._uninstall_claude)
        self.uninstall_btn.pack(side=tk.RIGHT, padx=12, pady=10)
        # Initially hidden — shown when claude is detected as installed
    ...
    def _uninstall_claude(self):
        """Confirm and uninstall Claude Code CLI."""
        if not messagebox.askyesno(_("btn_uninstall_claude"), _("uninstall_confirm")):
            return
        self.uninstall_btn.config(state=tk.DISABLED)
        self.uninstall_label.config(text=_("tool_installing"))

        def _run():
            r = uninstall_claude()
            if r["success"]:
                self.root.after(0, lambda: self.uninstall_label.config(
                    text=_("uninstall_success")))
                self.root.after(0, lambda: self.uninstall_frame.pack_forget())
                self.root.after(0, lambda: self._update_sys_step("claude",
                    STATUS_PENDING, ""))
            else:
                self.root.after(0, lambda: self.uninstall_label.config(
                    text=f"{_('uninstall_fail')}: {r.get('error', '')}"))
    ...
    def _build_page_apikey(self, page):
        ...
        # API key hint banner — soft reminder, not a blocker
        claude_hint = tk.Frame(page, bg="#E8F5E9", highlightbackground="#A5D6A7", highlightthickness=1)
        claude_hint.pack(fill=tk.X, padx=10, pady=(0, 6))
        tk.Label(claude_hint, text=f"💡  {_('claude_needs_anthropic')}",
                 font=("Microsoft YaHei", 9), bg="#E8F5E9", fg="#2E7D32",
                 anchor=tk.W, wraplength=860, justify=tk.LEFT).pack(fill=tk.X, padx=12, pady=6)
    ...
    def _on_provider_saved(self, provider_id):
        ...
        # Show restart hint after first key configuration
        if not hasattr(self, '_saved_providers'):
            self._saved_providers = set()
        is_new = provider_id not in self._saved_providers
        self._saved_providers.add(provider_id)
        if is_new:
            msg = _("restart_terminal")
            if provider_id == "anthropic":
                msg += "\n\n" + _("claude_first_step")
            messagebox.showinfo("✓ " + _("status_configured"), msg)
    ...
    def _build_page_done(self, page):
        ...
        # Quick start instructions — dynamic based on whether Anthropic is configured
        has_anthropic = self.configured.get("anthropic", False)
        guide = tk.Frame(center, bg="#E8F5E9" if has_anthropic else "#E3F2FD",
                        highlightbackground="#A5D6A7" if has_anthropic else "#90CAF9",
                        highlightthickness=1)
        ...
        if has_anthropic:
            ...
            tips = [
                "✓  API Keys saved — restart terminal to use them",
                "✓  Try: aider (pip install aider-chat) — works with 100+ models",
                "✓  Try: Continue.dev or Cline — VS Code AI extensions",
                "✓  Use Anthropic/OpenAI SDK directly in your scripts",
                "⚠  Claude Code CLI requires Anthropic API Key specifically",
            ]
        for tip in tips:
            tk.Label(guide, text=tip, font=("Microsoft YaHei", 10),
                     bg="#E8F5E9" if has_anthropic else "#E3F2FD",
                     fg=COLOR_TEXT, anchor=tk.W).pack(fill=tk.X, padx=16, pady=4)

        # ── Alternative tools section ──────────────────────────────
        alt_section = tk.Frame(center, bg=COLOR_BG)
        alt_section.pack(fill=tk.X, padx=40, pady=(16, 0))

        tk.Label(alt_section, text=_("alt_tools_title"), font=("Microsoft YaHei", 12, "bold"),
                 bg=COLOR_BG, fg=COLOR_TEXT).pack(anchor=tk.W)
        tk.Label(alt_section, text=_("alt_tools_desc"), font=("Microsoft YaHei", 9),
                 bg=COLOR_BG, fg=COLOR_MUTED).pack(anchor=tk.W, pady=(2, 8))

        for tool in ALTERNATIVE_TOOLS:
            self._create_tool_card(alt_section, tool)
    ...
    def _create_tool_card(self, parent, tool):
        """Create a single alternative tool card with install button."""
        installed = check_tool_installed(tool["verify"])

        card = tk.Frame(parent, bg=COLOR_CARD, highlightbackground="#E0E0E0",
                        highlightthickness=1)
        ...
        tk.Label(left, text=f"{tool['icon']}  {tool['name']}",
                 font=("Microsoft YaHei", 10, "bold"), bg=COLOR_CARD, fg=COLOR_TEXT).pack(anchor=tk.W)
        desc = tool.get("desc_zh", tool.get("desc_en", ""))
        tk.Label(left, text=desc, font=("Microsoft YaHei", 8), bg=COLOR_CARD,
                 fg=COLOR_MUTED, wraplength=500, justify=tk.LEFT).pack(anchor=tk.W, pady=(2, 0))

        right = tk.Frame(card, bg=COLOR_CARD)
        right.pack(side=tk.RIGHT, padx=12, pady=10)

        if installed:
            tk.Label(right, text=_("tool_installed"), fg=COLOR_DONE,
                     bg=COLOR_CARD, font=("Microsoft YaHei", 9, "bold")).pack()
        else:
            status_lbl = tk.Label(right, text="", fg=COLOR_MUTED, bg=COLOR_CARD,
                                  font=("Microsoft YaHei", 8))
            status_lbl.pack(pady=(0, 4))

            def _install():
                status_lbl.config(text=_("tool_installing"), fg=COLOR_RUNNING)
                btn.config(state=tk.DISABLED)

                def _run():
                    def progress(msg, pct):
                        pass
                    r = install_alternative_tool(tool["id"], progress)
                    if r["success"]:
                        status_lbl.config(text=_("tool_installed"), fg=COLOR_DONE)
                        btn.pack_forget()
                    else:
                        status_lbl.config(text=_("tool_install_fail"), fg=COLOR_ERROR)
                        btn.config(state=tk.NORMAL)

                threading.Thread(target=_run, daemon=True).start()

            btn = tk.Button(right, text=_("btn_install_tool"),
                           font=("Microsoft YaHei", 8), bg="#E3F2FD", fg="#1565C0",
