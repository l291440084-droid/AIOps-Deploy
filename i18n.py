...
_current_lang = "en"  # Default to English for universal readability
_config_dir = Path.home() / ".ai-deploy"
_config_file = _config_dir / "config.json"


def _detect_system_locale() -> str:
    """Auto-detect the best language from system locale. Returns language code."""
    import locale
    try:
...
def load_language():
    """Restore saved language preference, or auto-detect from system locale."""
    global _current_lang
    try:
    ...
    _current_lang = _detect_system_locale()
...
STRINGS = {
    ...
    "restart_terminal": {
        "en": "API key saved. Please RESTART your terminal for it to take effect.",
        "zh": "API Key 已保存。请重启终端使其生效。",
        "fr": "Clé API enregistrée. REDÉMARREZ le terminal pour qu'elle prenne effet.",
        "es": "Clave API guardada. REINICIE el terminal para que surta efecto.",
        "ru": "API ключ сохранен. ПЕРЕЗАПУСТИТЕ терминал.",
        "ar": "تم حفظ مفتاح API. أعد تشغيل الطرفية.",
    },
    "claude_needs_anthropic": {
        "en": "Claude Code requires Anthropic API Key. Skip if using other tools (OpenAI, DeepSeek, etc.)",
        "zh": "Claude Code 需要 Anthropic API Key。如使用其他工具 (OpenAI/DeepSeek 等) 可跳过此项。",
        "fr": "Claude Code nécessite une clé Anthropic. Ignorez si vous utilisez d'autres outils.",
        "es": "Claude Code requiere clave Anthropic. Omita si usa otras herramientas.",
        "ru": "Claude Code требует ключ Anthropic. Пропустите, если используете другие инструменты.",
        "ar": "يتطلب Claude Code مفتاح Anthropic. تخطَّ إذا كنت تستخدم أدوات أخرى.",
    },
    "claude_first_step": {
        "en": "After restarting terminal, run: claude   (Anthropic key required)",
        "zh": "重启终端后运行: claude   （需 Anthropic Key）",
        "fr": "Après redémarrage: claude   (clé Anthropic requise)",
        "es": "Tras reiniciar: claude   (requiere clave Anthropic)",
        "ru": "После перезапуска: claude   (требуется ключ Anthropic)",
        "ar": "بعد إعادة التشغيل: claude   (مطلوب مفتاح Anthropic)",
    },
    "skip_anthropic_alternatives": {
        "en": "Without Anthropic key, use other CLI tools with your configured keys:\n• aider (supports 100+ models)\n• openai-python / anthropic SDK directly\n• Continue.dev / Cline for VS Code",
        "zh": "不用 Anthropic 的话，可用你已配置的 Key 搭配其他工具:\n• aider — 支持 100+ 模型\n• 直接用 openai/anthropic SDK 写脚本\n• Continue.dev / Cline (VS Code 插件)",
        "fr": "Sans clé Anthropic, utilisez d'autres outils CLI avec vos clés.",
        "es": "Sin clave Anthropic, use otras herramientas CLI con sus claves.",
        "ru": "Без ключа Anthropic используйте другие CLI инструменты с вашими ключами.",
        "ar": "بدون مفتاح Anthropic، استخدم أدوات CLI أخرى مع مفاتيحك.",
    },
    "export_warning": {
    ...
    "alt_tools_title": {
        ...
        "zh": "🔧 替代工具推荐",
        "fr": "🔧 Outils Alternatifs",
        "es": "🔧 Herramientas Alternativas",
        "ru": "🔧 Альтернативные Инструменты",
        "ar": "🔧 أدوات بديلة",
    },
    "alt_tools_desc": {
        "en": "Use your configured API keys with these tools. No Anthropic account needed.",
        "zh": "用已配置的 API Key 搭配这些工具，无需 Anthropic 账号。",
        "fr": "Utilisez vos clés API avec ces outils. Pas besoin de compte Anthropic.",
        "es": "Use sus claves API con estas herramientas. Sin cuenta Anthropic.",
        "ru": "Используйте настроенные API ключи с этими инструментами.",
        "ar": "استخدم مفاتيح API المكونة مع هذه الأدوات. لا حاجة لحساب Anthropic.",
    },
    "btn_install_tool": {
        "en": "Install",
        "zh": "安装",
    ...
    "btn_uninstall_claude": {
        "en": "🗑 Uninstall Claude Code",
        "zh": "🗑 卸载 Claude Code",
        "fr": "🗑 Désinstaller Claude Code",
        "es": "🗑 Desinstalar Claude Code",
        "ru": "🗑 Удалить Claude Code",
        "ar": "🗑 إلغاء تثبيت Claude Code",
    },
    "uninstall_confirm": {
        "en": "Uninstall Claude Code CLI?\n\nThis runs: npm uninstall -g @anthropic-ai/claude-code",
        "zh": "确定卸载 Claude Code CLI？\n\n将运行: npm uninstall -g @anthropic-ai/claude-code",
        "fr": "Désinstaller Claude Code CLI?\n\nExécute: npm uninstall -g @anthropic-ai/claude-code",
        "es": "¿Desinstalar Claude Code CLI?\n\nEjecuta: npm uninstall -g @anthropic-ai/claude-code",
        "ru": "Удалить Claude Code CLI?\n\nЗапустит: npm uninstall -g @anthropic-ai/claude-code",
        "ar": "إلغاء تثبيت Claude Code CLI?\n\nسيشغل: npm uninstall -g @anthropic-ai/claude-code",
    },
    "uninstall_success": {
        "en": "Claude Code uninstalled successfully.",
        "zh": "Claude Code 已成功卸载。",
        "fr": "Claude Code désinstallé avec succès.",
        "es": "Claude Code desinstalado correctamente.",
        "ru": "Claude Code успешно удален.",
        "ar": "تم إلغاء تثبيت Claude Code بنجاح.",
    },
    "uninstall_fail": {
        "en": "Uninstall failed. Check console for details.",
        "zh": "卸载失败，请查看控制台了解详情。",
