...
ALTERNATIVE_TOOLS = [
    {
        ...
        "name": "Aider",
        "icon": "🤖",
        "cmd": [sys.executable, "-m", "pip", "install", "-U", "aider-chat"],
        "verify": "aider",
        "desc_en": "AI pair programming in terminal — supports 100+ models (DeepSeek, OpenAI, Anthropic, etc.)",
        "desc_zh": "终端 AI 结对编程工具 — 支持 100+ 模型 (DeepSeek/OpenAI/Anthropic 等)",
    ...
    },
    {
        ...
        "name": "Open WebUI",
        "icon": "🌐",
        "cmd": [sys.executable, "-m", "pip", "install", "-U", "open-webui"],
        "verify": "open-webui",
        "desc_en": "Self-hosted ChatGPT-style web UI — works with any OpenAI-compatible API",
        "desc_zh": "自托管 ChatGPT 风格 Web 界面 — 兼容任何 OpenAI API",
...
def install_alternative_tool(tool_id: str,
                              progress_callback: Optional[Callable[[str, float], None]] = None) -> dict:
    """Install an alternative AI CLI tool via pip."""
    tool = next((t for t in ALTERNATIVE_TOOLS if t["id"] == tool_id), None)
    if not tool:
        return fail(error=f"Unknown tool: {tool_id}")
    if progress_callback:
        progress_callback(f"Installing {tool['name']}...", 0.1)
    try:
        proc = subprocess.Popen(
            tool["cmd"],
        ...
        return fail(error=f"Failed to start pip: {e}", fix="Check Python/pip installation")
    output_lines = []
    for line in proc.stdout:
        line = line.rstrip()
        output_lines.append(line)
        if progress_callback and len(output_lines) % 3 == 0:
            progress_callback(line[:100], 0.1 + 0.8 * min(len(output_lines) / 50, 1.0))
    proc.wait()
    if proc.returncode != 0:
        return fail(
            error=f"pip install failed (exit {proc.returncode})",
            fix=f"Run manually: {' '.join(tool['cmd'])}",
            output="\n".join(output_lines[-10:]),
        )
    if progress_callback:
        progress_callback("Done", 1.0)
    installed = check_tool_installed(tool["verify"])
    return ok(installed=installed, tool=tool["name"], output="\n".join(output_lines[-5:]))


# ── Claude Code Uninstall ────────────────────────────────────────

def uninstall_claude(progress_callback: Optional[Callable[[str, float], None]] = None) -> dict:
    """Uninstall Claude Code CLI via npm."""
    npm = shutil.which("npm")
    if not npm:
        return fail(error="npm not found", fix="No npm available to uninstall")
    system = platform.system()
    if system == "Linux" and os.geteuid() != 0:
        try:
            prefix = subprocess.check_output([npm, "config", "get", "prefix"], text=True).strip()
            if Path(prefix).exists() and os.access(Path(prefix), os.W_OK):
                cmd = [npm, "uninstall", "-g", "@anthropic-ai/claude-code"]
            else:
                cmd = ["sudo", npm, "uninstall", "-g", "@anthropic-ai/claude-code"]
        except Exception:
            cmd = [npm, "uninstall", "-g", "@anthropic-ai/claude-code"]
    else:
        cmd = [npm, "uninstall", "-g", "@anthropic-ai/claude-code"]
    if progress_callback:
        progress_callback(f"Running: {' '.join(cmd)}", 0.1)
    try:
        proc = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, text=True, bufsize=1)
    except Exception as e:
        return fail(error=f"Failed to start npm: {e}")
    output_lines = []
    for line in proc.stdout:
        output_lines.append(line.rstrip())
    proc.wait()
    if proc.returncode != 0:
        return fail(error=f"Uninstall failed (exit {proc.returncode})", output="\n".join(output_lines[-10:]))
    if progress_callback:
        progress_callback("Uninstalled", 1.0)
    return ok(uninstalled=True, output="\n".join(output_lines[-5:]))
